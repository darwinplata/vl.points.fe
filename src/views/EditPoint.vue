...
const pointInfo = computed(() => pointsStore.getPointInfo(id));

// Form: Initial values
const formValues = {
  id: id,
  name: pointInfo.value?.name || '',
  x: pointInfo.value?.x || 0,
  y: pointInfo.value?.y || 0
};
...
const sortedPoints = computed(function () { return pointInfo ? pointsStore.sortPoints(listsValues.value) : null});
...
async function deleteValue(e) {
  e.preventDefault();
  await pointsStore.destroyPoint(id);
  backHome();
}
...
<template>
  ...
  <div v-if="pointInfo">
    ...
    <div>
      <h5>Nearest point<span v-if="nearestPoints.length > 1">s</span> at distance {{ nearestDistance &&
    ...
    <div id="farthest">
      <br>
      <h5 id="hFarTitle">Farthest point<span v-if="farthestPoints.length > 1">s</span> at distance {{ fartherDistance &&
          fartherDistance.toFixed(1)
      }}
      </h5>
      <table class="table table-striped">
        <thead class="thead-light">
